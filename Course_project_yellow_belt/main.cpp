...
std::pair <Date, std::string> Entry(Date a, std::string b) {
...
void TestsMyCustom()
{
...

	{
		...
		AssertEqual(p, db.Last({ 2019, 12, 4 }), " My test 6");
...
void TestParseEvent() {
	...
	}
	{
		...
		events.push_back(ParseEvent(is));
		AssertEqual(events, vector<std::string>{"first event  ", "second event"}, "Parse multiple events");
	}
}
...
void TestAll() {
	...
	tr.RunTest(TestEmptyNode, "test 2  Coursera");
	tr.RunTest(TestDbLast, "test 3(3) Coursera");
	tr.RunTest(TestDbFind, "test 3(2) Coursera");
	tr.RunTest(TestDbAdd, "test 3(1) Coursera");
	tr.RunTest(TestInsertionOrder, "test cout");
	tr.RunTest(TestDbRemoveIf, "test 3(4) Coursera");
	tr.RunTest(TestsMyCustom, "my  test");
	tr.RunTest(TestDatabase, "test  GitHub");
}
